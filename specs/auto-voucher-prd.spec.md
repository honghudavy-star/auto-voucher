...
- 部分完成：XML 已支持通用字段映射，仍需真实数电发票样本扩充；文本型 PDF 已提取文本但尚未提供与图片 OCR 相同的字段确认表单；本地账簿是已确认凭证汇总，不是法定总账报表；Windows Docker 交付仍需在真实 Windows 环境完成安装验收。
...
- 所有原始资料和人工修改可追溯；
- 在干净环境完成一次备份恢复；
- Windows Docker 一键启动可由非开发者完成安装和首次示例；
- 不需要安装 Git、Node.js、Python、API Key 或数据库配置。
...
5. **目标平台**：Windows 通过 Docker Desktop 交付，macOS/Linux 作为本地开发环境，是否需要额外的桌面发行版？
