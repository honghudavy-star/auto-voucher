...
func TestSuccessfulCoreInstallRemovesDownloadedArchive(t *testing.T) {
	root := t.TempDir()
	dirs := testLauncherDirectories(t, root)
	entrypoint := []byte("core executable")
	archive, pkg, server := signedTestPackage(t, "AutoVoucherCore.exe", entrypoint)
	defer server.Close()

	launcher := Launcher{
		client: server.Client(),
		dirs:   dirs,
		state:  State{Components: map[string]string{}},
	}
	manifest := validManifest()
	manifest.Version = "0.2.2"
	manifest.Core = pkg

	if err := launcher.install(&manifest); err != nil {
		t.Fatalf("core install failed: %v", err)
	}
	cachePath := filepath.Join(dirs.Cache, "core-0.2.2.zip.part")
	if _, err := os.Stat(cachePath); !os.IsNotExist(err) {
		t.Fatalf("successful core archive should be removed, stat error = %v", err)
	}
	installed, err := os.ReadFile(filepath.Join(dirs.Versions, "0.2.2", "AutoVoucherCore.exe"))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(installed, entrypoint) || len(archive) == 0 {
		t.Fatal("installed core differs from signed package")
	}
}

func TestSuccessfulComponentInstallRemovesArchiveAndStaleVersions(t *testing.T) {
	root := t.TempDir()
	dirs := testLauncherDirectories(t, root)
	stale := filepath.Join(dirs.Components, "ocr", "0.2.1")
	if err := os.MkdirAll(stale, 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(stale, "AutoVoucherOCR.exe"), []byte("old"), 0o600); err != nil {
		t.Fatal(err)
	}
	entrypoint := []byte("current OCR executable")
	_, pkg, server := signedTestPackage(t, "AutoVoucherOCR.exe", entrypoint)
	defer server.Close()
	...
	if err := launcher.installComponent("ocr", "0.2.2", pkg); err != nil {
		t.Fatalf("component install failed: %v", err)
	}
	cachePath := filepath.Join(dirs.Cache, "ocr-0.2.2.zip.part")
	if _, err := os.Stat(cachePath); !os.IsNotExist(err) {
		t.Fatalf("successful component archive should be removed, stat error = %v", err)
	}
	if _, err := os.Stat(stale); !os.IsNotExist(err) {
		t.Fatalf("stale component version should be removed, stat error = %v", err)
	}
	active := filepath.Join(dirs.Components, "ocr", "0.2.2", "AutoVoucherOCR.exe")
	if _, err := os.Stat(active); err != nil {
		t.Fatalf("active component was removed: %v", err)
	}
}

func TestCleanupInstalledPackagesKeepsIncompleteDownloads(t *testing.T) {
	root := t.TempDir()
	dirs := testLauncherDirectories(t, root)
	installedPaths := []string{
		filepath.Join(dirs.Versions, "0.2.2", "AutoVoucherCore.exe"),
		filepath.Join(dirs.Components, "ocr", "0.2.2", "AutoVoucherOCR.exe"),
		filepath.Join(dirs.Components, "ocr", "0.2.1", "AutoVoucherOCR.exe"),
	}
	...
	installedCaches := []string{
		filepath.Join(dirs.Cache, "core-0.2.2.zip.part"),
		filepath.Join(dirs.Cache, "ocr-0.2.2.zip.part"),
		filepath.Join(dirs.Cache, "ocr-0.2.1.zip.part"),
	}
	for _, path := range installedCaches {
		if err := os.WriteFile(path, []byte("cache"), 0o600); err != nil {
			t.Fatal(err)
		}
	}
	incomplete := filepath.Join(dirs.Cache, "core-0.3.0.zip.part")
	...
	launcher := Launcher{
		...
		state: State{
			...
			Components:     map[string]string{"ocr": "0.2.2"},
		},
	}

	removedFiles, freedBytes, err := launcher.cleanupInstalledPackages()
	if err != nil {
		t.Fatalf("cleanup failed: %v", err)
	}
	if removedFiles != len(installedCaches) || freedBytes != int64(len("cache")*len(installedCaches)) {
		t.Fatalf("cleanup result = %d files, %d bytes", removedFiles, freedBytes)
	}
	for _, path := range installedCaches {
		if _, err := os.Stat(path); !os.IsNotExist(err) {
			t.Fatalf("installed package cache was retained: %s", path)
		}
	}
	if _, err := os.Stat(incomplete); err != nil {
		t.Fatalf("incomplete resumable download was removed: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dirs.Components, "ocr", "0.2.1")); !os.IsNotExist(err) {
		t.Fatalf("stale OCR version should be removed, stat error = %v", err)
	}
}
...
func testLauncherDirectories(t *testing.T, root string) Directories {
	t.Helper()
	dirs := Directories{
		Root:       root,
...
func signedTestPackage(
	t *testing.T,
	entrypoint string,
	entrypointBody []byte,
) ([]byte, Package, *httptest.Server) {
	t.Helper()
	...
	if _, err := entry.Write(entrypointBody); err != nil {
		t.Fatal(err)
	}
	...
	entrypointDigest := sha256.Sum256(entrypointBody)
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, _ *http.Request) {
	...
	return archive, Package{
		...
		ExpandedSize:     int64(len(entrypointBody)),
		SHA256:           hex.EncodeToString(archiveDigest[:]),
		Signature:        base64.StdEncoding.EncodeToString(ed25519.Sign(privateKey, archiveDigest[:])),
		Entrypoint:       entrypoint,
		EntrypointSHA256: hex.EncodeToString(entrypointDigest[:]),
		DatabaseLevel:    2,
	}, server
}
